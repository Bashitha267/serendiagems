import Link from "next/link";
import Image from "next/image";

export default function Footer() {
    return (
        <footer className="bg-gray-50 border-t border-gray-200 mt-auto">
            <div className="max-w-[1440px] mx-auto px-6 lg:px-20 py-16">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                    {/* Brand */}
                    <div className="flex flex-col gap-6">
                        <div className="flex items-center gap-3 text-black">
                            <div className="size-6 text-black">
                                <img
                                    src="https://res.cloudinary.com/dnfbik3if/image/upload/v1769443860/WhatsApp_Image_2026-01-20_at_22.23.34_f0nnpp.jpg"
                                    alt="Ceylon Gems Logo"
                                    width={64}
                                    height={64}
                                />
                            </div>
                            <h2 className="text-xl font-serif font-bold">Serendia Gems</h2>
                        </div>
                        <p className="text-black/60 text-sm leading-relaxed">
                            Exporting the finest Sri Lankan gemstones to the world with
                            integrity, transparency, and a commitment to our heritage.
                        </p>
                        <div className="flex gap-4 mt-2">
                            <a
                                href="https://www.instagram.com"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-black/40 hover:text-[#E4405F] transition-colors"
                                aria-label="Instagram"
                            >
                                <svg className="size-5 fill-current" viewBox="0 0 24 24">
                                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.584.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                                </svg>
                            </a>
                            <a
                                href="https://www.facebook.com"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-black/40 hover:text-[#1877F2] transition-colors"
                                aria-label="Facebook"
                            >
                                <svg className="size-5 fill-current" viewBox="0 0 24 24">
                                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                                </svg>
                            </a>

                        </div>
                    </div>

                    {/* Links - Collections */}
                    <div className="flex flex-col gap-4">
                        <h3 className="text-black font-bold text-sm tracking-widest uppercase">
                            Collections
                        </h3>
                        <Link
                            href="/collections"
                            className="text-black/60 hover:text-[#1152d4] transition-colors text-sm"
                        >
                            Blue Sapphires
                        </Link>
                        <Link
                            href="/collections"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Rubies
                        </Link>
                        <Link
                            href="/collections"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Padparadscha
                        </Link>
                        <Link
                            href="/collections"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Engagement Rings
                        </Link>
                    </div>

                    {/* Links - Company */}
                    <div className="flex flex-col gap-4">
                        <h3 className="text-black font-bold text-sm tracking-widest uppercase">
                            Company
                        </h3>
                        <Link
                            href="/about"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Our Story
                        </Link>
                        <Link
                            href="/about"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Ethical Mining
                        </Link>
                        <Link
                            href="/about"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Certification (GIA/GRS)
                        </Link>
                        <Link
                            href="/about"
                            className="text-black/60 hover:text-black transition-colors text-sm"
                        >
                            Contact Us
                        </Link>
                    </div>

                    {/* Newsletter */}
                    <div className="flex flex-col gap-4">
                        <h3 className="text-black font-bold text-sm tracking-widest uppercase">
                            Stay Updated
                        </h3>
                        <p className="text-black/60 text-sm">
                            Subscribe to receive exclusive offers and gemological insights.
                        </p>
                        <div className="flex gap-2">
                            <input
                                className="bg-white border border-gray-300 rounded-lg px-4 py-2 text-black text-sm w-full focus:outline-none focus:border-black transition-colors"
                                placeholder="Email address"
                                type="email"
                            />
                            <button className="bg-[#1152d4] hover:bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-bold transition-colors">
                                Join
                            </button>
                        </div>
                    </div>
                </div>

                <div className="border-t border-gray-200 mt-16 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                    <p className="text-black/30 text-xs">
                        © 2024 Serendia Gems. All rights reserved.
                    </p>
                    <div className="flex gap-6">
                        <span className="text-black/30 text-xs hover:text-black cursor-pointer transition-colors">
                            Privacy Policy
                        </span>
                        <span className="text-black/30 text-xs hover:text-black cursor-pointer transition-colors">
                            Terms of Service
                        </span>
                        <span className="text-black/30 text-xs hover:text-black cursor-pointer transition-colors">
                            Shipping &amp; Returns
                        </span>
                    </div>
                </div>
            </div>
        </footer>
    );
}
